...
import (
	...
	"github.com/consensys/orchestrate/src/tx-listener/service/listener"
...
const chainPendingTxsComponent = "tx-listener.data-puller.chain-pending-jobs"
const chainPendingTxsSessionComponent = "tx-listener.data-puller.chain-pending-jobs.session"

type chainPendingJobsListener struct {
	client                 orchestrateclient.OrchestrateClient
	logger                 *log.Logger
	txlistener             listener.Txlistener
	onChainPendingJobEvent usecases.PendingJobUseCase
...
func (l *chainPendingJobsListener) Run(ctx context.Context) error {
	...
	go func() {
		...
		subscriptionID := l.txlistener.Subscribe(chainEvents)
		...
		_ = l.txlistener.Unsubscribe(subscriptionID)
	}()
...
func (l *chainPendingJobsListener) addChainSession(ctx context.Context, chain *entities.Chain) error {
	...
	sess := newChainPendingJobSession(l.client, l.onChainPendingJobEvent, chain, l.interval, l.logger)
