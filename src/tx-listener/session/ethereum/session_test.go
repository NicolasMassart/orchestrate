...
import (
	...
	mock3 "github.com/consensys/orchestrate/src/tx-listener/session/ethereum/mocks"
	...
	mock5 "github.com/consensys/orchestrate/src/tx-listener/metrics/mock"
	"github.com/consensys/orchestrate/src/tx-listener/session/ethereum/hooks/mock"
	mock2 "github.com/consensys/orchestrate/src/tx-listener/session/ethereum/offset/mock"
...
func TestSession_Run(t *testing.T) {
	...
	t.Run("should process block successfully with internal txs", func(t *testing.T) {
		...
		mockClient.EXPECT().
			...
			Return([]*apitypes.JobResponse{jobResponse}, nil)
	...
	t.Run("should process block successfully with internal txs in batches", func(t *testing.T) {
		...
		mockClient.EXPECT().
			...
			Return([]*apitypes.JobResponse{}, nil)
		mockClient.EXPECT().
			SearchJob(gomock.Any(), &entities.JobFilters{
	...
	t.Run("should fetch receipts successfully an ignore private receipt", func(t *testing.T) {
		...
		bckoff := &backoffmock.MockIntervalBackoff{}
		session.bckOff = bckoff
		err := errors.InvalidParameterError("private receipt not found")
	...
	t.Run("should not fail if PrivateTransactionReceipt fails", func(t *testing.T) {
		...
		mockClient.EXPECT().
			...
			Return([]*apitypes.JobResponse{}, nil).AnyTimes()
