...
func TestSendEEAPrivate_Execute(t *testing.T) {
	...
	usecase := NewSendEEAPrivateTxUseCase(signTx, crafter, ec, msgAPI, chainRegistryURL, nonceManager)
	...
	t.Run("should execute use case successfully", func(t *testing.T) {
		...
		msgAPI.EXPECT().JobUpdateMessage(gomock.Any(), 
			testdata2.SentJobMessageRequestMatcher(job.UUID, entities.StatusStored, &txHash), gomock.Any()).Return(nil)
