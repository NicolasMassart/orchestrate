...
type listenBlockListener struct {
	...
	onChainBlockEvent usecases.ChainBlockEventHandler
...
func (s *listenBlockSession) Start(ctx context.Context) error {
	s.logger.Info("chain block listener started")
...
func (s *listenBlockSession) Update(chain *entities.Chain) error {
	...
	s.chain = chain
	// @TODO Evaluate consequences of a chain update.
	// For instance update ticker interval
	return nil
}
...
func (s *listenBlockSession) triggerEvents(ctx context.Context, blockEvents []*events.Block) error {
	...
	for _, event := range blockEvents {
		if err := s.onChainBlockEvent.Execute(ctx, event.Number, event.TxHashes); err != nil {
			return err
		}
	}
...
func (s *listenBlockSession) retrieveBlocks(ctx context.Context, curBlockNumber uint64) ([]*events.Block, uint64, error) {
	...
	select {
	case err := <-cerr:
		return newBlockEvents, curBlockNumber, err
	case <-ctx.Done():
		return newBlockEvents, curBlockNumber, ctx.Err()
	case <-cdone:
