...
type chainsService struct {
	...
	addChainEvent    usecases.NewChainEventHandler
	updateChainEvent usecases.UpdateChainEventHandler
	deleteChainEvent usecases.DeleteChainEventHandler
...
func (s *chainsService) Run(ctx context.Context) error {
	s.logger.Info("chains listener started")
