...
type retryJobSessionHandler struct {
...
func RetrySessionHandler(client orchestrateclient.OrchestrateClient,
	retrySessionJobUseCase usecases.SendRetryJob,
	retrySessionState state.RetrySessions,
	logger *log.Logger,
) usecases.SessionHandler {
	return &retryJobSessionHandler{
...
func (uc *retryJobSessionHandler) StartSession(ctx context.Context, job *entities.Job) error {
	sessID := job.UUID
...
func (uc *retryJobSessionHandler) StopSession(ctx context.Context, sessID string) error {
...
func (uc *retryJobSession) Start(ctx context.Context) error {
	...
	go func() {
		err := backoff.RetryNotify(
			...
			func(err error, d time.Duration) {
				uc.logger.WithError(err).Warnf("error in job retry session, restarting in %v...", d)
			},
		)

		if err != nil {
			uc.logger.WithError(err).Error("existed with errors")
			uc.cerr <- err
			return
		}
	...
	select {
	case err := <-uc.cerr:
		uc.logger.WithField("err", err).Info("exited with errors")
		return err
	case <-ctx.Done():
...
func (uc *retryJobSession) runSession(ctx context.Context, ses *retryJobSessionData) error {
	...
	for {
		select {
		case <-ticker.C:
			childJobUUID, err := uc.retrySessionJobUseCase.Execute(ctx, ses.parentJob.UUID, ses.lastChildJobUUID, ses.nChildren)
			if err != nil {
