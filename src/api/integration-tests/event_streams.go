...
func (s *eventStreamsTestSuite) TestUpdate() {
	...
	s.T().Run("should fail to update event stream if chain does not exists", func(t *testing.T) {
		updateReq := testdata.FakeUpdateWebhookEventStreamRequest()
		updateReq.Chain = "invalidChain"

		_, err = s.client.UpdateWebhookEventStream(ctx, esKafka.UUID, updateReq)
		require.Error(t, err)
		assert.Equal(t, http.StatusUnprocessableEntity, err.(*client.HTTPErr).Code())
	})
}
